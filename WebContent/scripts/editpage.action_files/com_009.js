...
var tinymce = {
	...
	_init : function() {
        ...
        // ATLASSIAN - we set the base url later on below with AJS.toInit()
        tinymce.baseURL = "";
        tinymce.confluence = {};
	},
...
// ATLASSIAN - this pre-existing call to _init is important because other initialisation code in this script relies on it.
// Don't try to delay this call.
// Initialize the API
tinymce._init();

// ALTASSIAN - We can delay the setting of the baseURL until after dom ready as long as its the first thing we do
// before tinymce needs it.
AJS.toInit(function() {
    tinymce.baseURL = AJS.Editor.Adapter.getTinyMceBaseUrl();
    console.log("tinymce.baseURL set:" + tinymce.baseURL);
});
	...
	tinymce.create('tinymce.util.URI', {
		URI : function(u, s) {
			...
			if (u.indexOf('/') === 0 && u.indexOf('//') !== 0) {
                console.log("URI.URI Absolute path with no host u : " + u);
				u = (s.base_uri ? s.base_uri.protocol || 'http' : 'http') + '://mce_host' + u;
            }
			// Relative path
			if (u.indexOf('://') === -1 && u.indexOf('//') !== 0) {
                console.log("URI.URI relative path with no host u : " + u);
...
(function() {
	...
	tinymce.create('static tinymce.dom.Event', {
		...
		_pageInit : function() {
            console.log("Event:_pageInit starting");
			var e = Event;
...
(function() {
    ...
    // ATLASSIAN - need to catch Ps with BR only, as well as #text, when walking the DOM tree.
	var textAndEmptyParagraphFilter = {
        acceptNode: function(node) {
            ...
            // Otherwise accept if the node is a P with one BR inside
            return (node.nodeName.toLowerCase() == "p" && node.childNodes.length == 1 &&
	...
	tinymce.create('tinymce.dom.Selection', {
		...
		getBookmark : function(si) {
			...
			function getPos(r, sn, en) {
                // ATLASSIAN - use filter that finds paragraphs with only BRs as well as text nodes
                var w = t.dom.doc.createTreeWalker(r, NodeFilter.SHOW_ALL, textAndEmptyParagraphFilter, false), n, p = 0, d = {};
		...
		moveToBookmark : function(b) {
			...
			function getPos(r, sp, ep) {
                // ATLASSIAN - use filter that finds paragraphs with only BRs as well as text nodes
                var w = t.dom.doc.createTreeWalker(r, NodeFilter.SHOW_ALL, textAndEmptyParagraphFilter, false), n, p = 0, d = {}, o, v, wa, wb;
	...
	tinymce.create('tinymce.ui.ListBox:tinymce.ui.Control', {
		...
		renderMenu : function() {
			...
			m = t.settings.control_manager.createDropMenu(t.id + '_menu', {
                ...
                // ATLASSIAN - don't need to limit the size of our menus, we don't have anything huge
                //max_width : 150,
				//max_height : 150
			});
...
(function() {
	...
	tinymce.create('tinymce.AddOnManager', {
		...
		load : function(n, u, cb, s) {
			var t = this;
            console.log("AddOnManager:load WARNING attempting to load: " + n);
...
(function() {
	...
	tinymce.create('static tinymce.EditorManager', {
		...
		preInit : function() {
            console.log("EditorManager.preInit");
			var t = this, lo = window.location;

            //ATLASSIAN - Use the TinyMCE plugin url as the document base
            // Setup some URLs where the editor API is located and where the document is
			tinymce.documentBaseURL = AJS.Editor.Adapter.getTinyMceBaseUrl();
            console.log("EditorManager.preInit tinymce.documentBaseURL=" + tinymce.documentBaseURL);
			...
			if (tinymce.EditorManager.baseURI.host != lo.hostname && lo.hostname) {
                ...
                console.log("EditorManager.preInit setting up document domain=" + tinymce.relaxedDomain);
            }
		...
		init : function(s) {
            console.log("EditorManager.init");
            ...
            // apply to us, so we just execute it.
            {
				...
				if (s.browsers) {
					...
					// Not a valid one
					if (!l) {
                        console.log("EditorManager:init Not a valid browser");
                        return;
                    }
                }
				switch (s.mode) {
    ...
    // ATLASSIAN - Done in tinyMceAdapter.js
...
(function() {
	...
	tinymce.create('tinymce.Editor', {
		Editor : function(id, s) {
            ...
            console.log("Editor:create t.documentBaseURI=" + t.documentBaseURI.getURI());
		...
		render : function(nst) {
            console.log("Editor:render starting");
...
//			}
			...
			if (!t.getElement()) {
			    console.log("Editor:render no editor element");
                return;
            }
            ...
            // ATLASSIAN - Don't call this - scripts are all loaded via #requireResource
			// Load scripts
            ...
            if (s.plugins.indexOf('compat2x') != -1) {
                ...
                // ATLASSIAN - Rather than calling loadScripts, we just call the t.init() part of it.
                //loadScripts();
                if (!t.removed)
                    t.init();
            }
		...
		init : function() {
            ...
            console.log("Editor:init iframe base href=" + AJS.Editor.Adapter.getCurrentBaseUrl());
			t.iframeHTML = s.doctype + '<html><head xmlns="http://www.w3.org/1999/xhtml"><base href="' + AJS.Editor.Adapter.getCurrentBaseUrl() + '" />';
            ...
            console.log("Editor:init finished rendering editor!!!");
		},

		setupIframe : function() {
			var t = this, s = t.settings, e = DOM.get(t.id), d = t.getDoc(), h, b;
            console.log("Editor:setupIframe isIE=" + isIE);
            ...
            console.log("Editor:setupIframe after post render");
			...
			setTimeout(function () {
				...
				if (s.content_css) {
					tinymce.each(explode(s.content_css), function(u) {
                        console.log("About to load content css: " + t.documentBaseURI.toAbsolute(u));
...
/*				t.onSetContent.add(function(ed, o) {
			...
			if (s.custom_shortcuts) {
				...
				for (i=1; i<=6; i++)
					t.addShortcut('ctrl+' + i, '', ['FormatBlock', false, 'h' + i]); // ATLASSIAN - our format block method expects html tag names
...
/*				ed.onPreProcess.add(function(ed, o) {
		...
		forceRoots : function(ed, e) {
			...
			// Fix for bug #1863847
            // Atlassian - replaced as the random jumping has been fixed (see comments below)
            // Pulled from tinyMCE 3.2 code.
            //if (e && e.keyCode == 13)
			//	return true;
				...
				if (nx.nodeType == 3 || (!t.dom.isBlock(nx) && nx.nodeType != 8)) {
					if (!bl) {
						...
						if (nx.nodeType != 3 || /[^\s]/g.test(nx.nodeValue)) {
							...
							if (si == -2 && r) {
								if (!isIE) {
                                    // Atlassian works out the selected element so we can go back
...
//				return true;
            ...
            if (y < vp.y || y + ch > vp.y + vp.h) {
				ed.getWin().scrollTo(0, y < vp.y ? y : (y + ch) - vp.h);
				//console.log('SCROLL!', 'vp.y: ' + vp.y, 'y' + y, 'vp.h' + vp.h, 'clientHeight' + ch, 'yyy: ' + (y < vp.y ? y : (y + ch) - vp.h));
			}
...
(function() {
	...
	tinymce.create('tinymce.WindowManager', {
		...
		open : function(s, p) {
            ...
            console.log("WindowManager:open popup name=" + s.name);
		...
		init : function(ed, url) {
			...
			ed.addCommand('mceFullScreen', function() {
				...
				if (ed.getParam('fullscreen_new_window')) {
					...
					n = DOM.add(DOM.doc.body, 'div', {id : 'mce_fullscreen_container', style : 'position:absolute;top:0;left:0;width:' + vp.w + 'px;height:' + vp.h + 'px;z-index:500;'});
...
(function() {
	...
	tinymce.create('tinymce.plugins.ContextMenu', {
		init : function(ed) {
            ...
            // Button may not be created by this point so use a timer.
            setTimeout(function() {
                var b = button();
                if (b.length) {
            ...
            ed.addButton('contextmenu', {title : 'contextmenu.desc', cmd : 'mceContextMenu'});
        },
...
//			m.add({title : 'advanced.paste_desc', icon : 'paste', cmd : 'Paste'});
            ...
            m.add({title : 'confluence.conflink_desc', icon : 'conflink', cmd : 'mceConflink', ui : true});
            if (el.nodeName == 'A') {
			...
			m.add({title : 'confluence.conf_macro_browser_desc', icon : 'conf_macro_browser', cmd : 'mceConfMacroBrowser', ui : true});
            m.addSeparator();
